...
public interface RadioStabilityRepository extends Neo4jRepository<RadioStability,Long> {
    ...
    @Query("MATCH (n:推进剂配方)-[r:hasCombustionProperty]->(m:辐射敏感性) where n.id = :prescriptionId return m")
    Optional<RadioStability> findRadioStabilityByPrescription(@Param("prescriptionId") Long prescriptionId);

    @Query("MATCH (n:辐射敏感性) where n.name = :stabilityName return n")
    Optional<RadioStability> findRadioStabilityByName(@Param("stabilityName")String stabilityName);
}
