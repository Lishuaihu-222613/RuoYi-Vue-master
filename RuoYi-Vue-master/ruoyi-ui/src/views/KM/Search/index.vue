  ...
  <div class="mind-box">
    ...
    <el-scrollbar class="mind-l">
      <div class="ml-m">
        ...
        <div class="ml-a-box" style="min-height:280px">
          <a @click="matchDomainGraph(m, $event)" v-for="(m, index) in pageModel.nodeList" :key="index" href="javascript:void(0)" :title="m.name">
            <el-tag v-if="m.commend == 0" closable style="margin:2px" @close="deleteDomain(m.id, m.name)">{{ m.name }}</el-tag>
          </a>
        </div>
        <div class="fr">
          <a href="javascript:void(0)" class="svg-a-sm" v-show="pageModel.pageIndex > 1" @click="prev">上一页</a>
          <a href="javascript:void(0)" class="svg-a-sm" v-show="pageModel.pageIndex < pageModel.totalPage" @click="next">下一页</a>
        </div>
      </div>
    </el-scrollbar>
    <!-- 左侧over -->
    ...
    <div class="mind-con">
      ...
      <div class="mind-top clearfix">
        <div v-show="domain != ''" class="fl" style="display: flex">
          ...
          <span>
            <span class="dibmr">
              <span>显示节点个数:</span>
              <a v-for="(m, index) in pageSizeList" :key="index" @click="setMatchSize(m)" :title="m.size" href="javascript:void(0)" :class="[m.isActive ? 'sd-active' : '', 'sd']">{{ m.size }}</a>
            </span>
          </span>
        </div>
        <div class="fr">
      ...
      <div>
        ...
        <!-- 中部图谱画布 -->
        <div>
          <div id="graphContainer" class="graphContainer" ref="menu_blank" @click="initContainerLeftClick" @contextmenu.prevent="initContainerRightClick"></div>
        </div>
      </el-scrollbar>
      </div>
      <!-- 中部over -->
    ...
    <!-- 空白处右键 -->
    <div>
      <menu-blank ref="menu_blank" @btnAddSingle="btnAddSingle" @btnQuickAddNode="btnQuickAddNode"></menu-blank>
    </div>
    <!-- 连线按钮组 -->
    <div>
      <menu-link ref="menu_link" @updateLinkName="updateLinkName" @deleteLink="deleteLink"></menu-link>
    </div>
    <!--编辑窗口-->
    <div>
      <kg-form ref="kg_form" @batchCreateNode="batchCreateNode" @batchCreateChildNode="batchCreateChildNode" @batchCreateSameNode="batchCreateSameNode" @createNode="createNode" @initNodeImage="initNodeImage" @initNodeContent="initNodeContent" @getDomain="getDomain">
      </kg-form>
    </div>
    <!-- 富文本展示 -->
    <div>
...
import MenuBlank from "@/views/KM/Search/components/menu_blank";
import MenuLink from "@/views/KM/Search/components/menu_link";
import KgForm from "@/views/KM/Search/components/kg_form";
import NodeRicher from "@/views/KM/Search/components/node_richer";
import KgJson from "@/views/KM/Search/components/kg_json";
