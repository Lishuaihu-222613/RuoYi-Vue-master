      ...
      <el-col :span="20" :xs="24">
        ...
        >
          ...
          <el-table-column v-if="columns[12].visible" key="clampElement" :show-overflow-tooltip="true"
                           align="center"
                           label="测量精度" prop="measureAccuracy"
          />
          <el-table-column v-if="columns[13].visible" key="clampStyle" :show-overflow-tooltip="true"
                           align="center"
                           label="测量高度" prop="measureHeight"
          />
          <el-table-column v-if="columns[14].visible" key="toolCapacity" :show-overflow-tooltip="true"
                           align="center"
          ...
          <el-table-column v-if="columns[15].visible" key="suitableProcesses" :show-overflow-tooltip="true"
                           align="center"
  ...
  data() {
    return {
      ...
      upload: {
        ...
        // 上传的地址
        url: process.env.VUE_APP_BASE_API + '/MeasuringTool/importData'
      },
      queryParams: {
        pageNum: 1,
        pageSize: 9,
      ...
      columns: [
        ...
        { key: 13, label: `测量高度`, visible: true },
        { key: 14, label: `工具能力`, visible: true },
        { key: 15, label: `适用工艺`, visible: true }
      ],
      title: '',
  ...
  methods: {
    ...
    getList() {
      ...
      if (this.queryParams.dynamicLabel === '') {
        resourceManagement.getAllMeasuringTools(this.queryParams).then(result => {
            if (result.code === 200) {
    ...
    /** 查询知识下拉树结构 */
    getTreeselect() {
      treeManagement.getTreeManagement(25819).then(response => {
      ...
      this.$refs["resource"].validate(valid => {
        if (valid) {
          if (this.selectResource.resourceId !== undefined) {
            resourceManagement.updateMeasuringTool(this.selectResource).then(response => {
              this.$modal.msgSuccess("修改成功");
              this.open = false;
              this.getList();
            });
          } else {
            resourceManagement.createMeasuringTool(this.selectResource).then(response => {
