<template>
  ...
  <div class="app-container">
    <el-form :model="queryParams" ref="queryForm" size="small" :inline="true" v-show="showSearch" label-width="68px">
      ...
      <el-form-item label="${comment}" prop="nodecount">
        <el-input
      ...
      <el-form-item label="${comment}" prop="shipcount">
        <el-input
    ...
    >
      <el-table-column label="${comment}" prop="name" />
      <el-table-column label="0=手动创建，1=三元组导入，2=excel导入，3=er图构建" align="center" prop="type" />
      <el-table-column label="概念描述" align="center" prop="description" />
      <el-table-column label="父级概念" align="center" prop="parentId" />
      <el-table-column label="${comment}" align="center" prop="nodecount" />
      <el-table-column label="${comment}" align="center" prop="shipcount" />
      ...
      <el-table-column label="更新者" align="center" prop="modifyuser" />
      <el-table-column label="更新时间" align="center" prop="modifytime" width="180">
        <template slot-scope="scope">
          <span>{{ parseTime(scope.row.modifytime, '{y}-{m}-{d}') }}</span>
        </template>
      </el-table-column>
      <el-table-column label="推荐" align="center" prop="commend" />
    ...
    <el-dialog :title="title" :visible.sync="open" width="500px" append-to-body>
      <el-form ref="form" :model="form" :rules="rules" label-width="80px">
        ...
        <el-form-item label="父级概念" prop="parentId">
          <treeselect v-model="form.parentId" :options="domainOptions" :normalizer="normalizer" placeholder="请选择父级概念" />
        </el-form-item>
        <el-form-item label="${comment}" prop="nodecount">
          <el-input v-model="form.nodecount" placeholder="请输入${comment}" />
        </el-form-item>
        <el-form-item label="${comment}" prop="shipcount">
          <el-input v-model="form.shipcount" placeholder="请输入${comment}" />
        </el-form-item>
        <el-form-item label="${comment}" prop="createuser">
  ...
  data() {
    return {
      ...
      rules: {
        ...
        type: [
          { required: true, message: "0=手动创建，1=三元组导入，2=excel导入，3=er图构建不能为空", trigger: "change" }
        ],
        parentId: [
  ...
  methods: {
    ...
    getList() {
      ...
      listDomain(this.queryParams).then(response => {
        this.domainList = this.handleTree(response.data, "id", "parentId");
        this.loading = false;
      });
    },
    /** 转换知识树管理数据结构 */
